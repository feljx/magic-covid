...
// Zeitpunk factory
//
export default function Zeitpunk(dateOrMs) {
    ...
    const toZeitops = (op) => (a, [k, v]) => ((a[k] = Zeitop(zeit, op, v)) || 1) && a;
...
// Helpers
//
function unixToDays(date = new Date()) {
    const ms = date instanceof Date
    ...
    const yoe = (doe - doe / BigInt(1460) + doe / BigInt(36524) - doe / BigInt(146096)) /
        BigInt(365);
